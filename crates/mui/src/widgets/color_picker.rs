//! One OKLCH color, several views. Layout belongs to MUI; only color geometry uses coordinates.
use std::sync::Arc;
...
/// Keep this beside the edited value, as for any MUI text field.
...
fn tabs<T: Copy + PartialEq>(ui: &mut Ui, id: &str, selected: &mut T, choices: &[(T, &str)]) -> El {
    row(choices.iter().map(|&(item, name)| {
        let (button, click) = super::button(ui, format!("{id}/{name}"), name);
        if click {
            *selected = item;
        }
        button
            .size(SpacingToken::S)
            .role(if *selected == item {
                Role::Raised
            } else {
                Role::Ink
            })
            .variant(if *selected == item {
                super::Variant::Solid
            } else {
                super::Variant::Ghost
            })
...
fn field(ui: &mut Ui, id: &str, state: &mut ColorPicker) -> El {
...
/// Intrinsic panel; `Color` stays in OKLCH while CSS input accepts any supported CSS Color 4 syntax.
pub fn color_picker(ui: &mut Ui, id: &str, state: &mut ColorPicker) -> (El, bool) {
    ...
    let (input, edited) = super::text_input(ui, &input_id, &mut state.draft);
    if edited {
        if let Some(c) = parse(&state.draft) {
            state.color = c;
        }
    }
    ...
    let rows: Vec<_> = channels
        .map(|i| {
            let (control, moved) = super::slider(
                ui,
                format!("{id}/{}", names[i]),
                names[i],
                &mut values[i],
    ...
    let swatch = leaf(28.0, 28.0).radius(8.0).fill(state.color);
    let header = row([
    ...
    sections.push(column([format_tabs, input, column(rows).gap(S)]).gap(S));
    let panel = column(sections)
        .gap(L)
        .pad(L)
        .preset(super::presets::panel())
        .shadow(Shadow::soft(16.0))
        .id(id);
    (panel, state.color != before)
}
...
mod tests {
    ...
    fn formats_and_shapes_resolve() {
        ...
        for shape in [
            ...
            picker.shape = shape;
            let tree = color_picker(&mut ui, "picker", &mut picker).0;
            ui.frame(
                tree,
