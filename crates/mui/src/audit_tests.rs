...
fn gain() -> El {
    leaf(120.0, 40.0).id("gain").focusable()
        .role(Kind::Slider { value: 0.5, min: 0.0, max: 1.0 })
}
fn step(ui: &mut Ui, tree: El, pos: Option<Point>, buttons: Buttons) -> Vec<(String, Edit)> {
    ui.frame(tree, None, PointerInput { pos, buttons, ..Default::default() }, 0.016)
        .unwrap().edits
}
...
fn rounded_clip_rejects_an_invisible_child_corner() {
    ...
    let tree = || stack![leaf(100.0, 100.0).id("child")].square(100.0).pill().clip();
    idle(&mut ui, tree());
    step(&mut ui, tree(), Some(Point::new(2.0, 2.0)), Buttons::PRIMARY);
    assert!(!ui.get("child").pressed);
    ui.cancel();
    step(&mut ui, tree(), Some(Point::new(50.0, 50.0)), Buttons::PRIMARY);
    assert!(ui.get("child").pressed);
}
...
fn removing_a_held_control_ends_its_gesture_in_that_frame() {
    ...
    let edits = step(&mut ui, leaf(120.0, 40.0), Some(Point::new(20.0, 20.0)), Buttons::PRIMARY);
...
fn disabling_a_held_control_ends_it_without_waiting_for_another_frame() {
    ...
    idle(&mut ui, gain());
    press(&mut ui);
    let edits = step(&mut ui, gain().disabled(true), Some(Point::new(20.0, 20.0)), Buttons::PRIMARY);
    assert_eq!(edits, vec![("gain".into(), Edit::End)]);
    assert!(!ui.get("gain").held);
}
...
fn invalid_time_does_not_poison_the_runtime() {
    ...
    for dt in [f64::NAN, f64::INFINITY, -0.001] {
        assert!(matches!(ui.frame(gain(), None, Input::default(), dt), Err(SceneError::InvalidFrameDelta)));
...
fn semantic_set_value_is_clamped_and_bracketed() {
    ...
    assert_eq!(value, 1.0);
    assert_eq!(idle(&mut ui, gain()), vec![("gain".into(), Edit::Begin), ("gain".into(), Edit::End)]);
    assert_eq!(ui.edits_for("gain").collect::<Vec<_>>(), vec![Edit::Begin, Edit::End]);
...
fn failed_layout_does_not_replay_a_consumed_activation() {
    ...
    assert!(ui.request_action(SemanticAction::activate("go")));
    assert!(ui.get("go").clicked);
    assert!(ui.frame(leaf(-1.0, 10.0), None, Input::default(), 0.016).is_err());
    assert!(!ui.get("go").clicked);
    assert_eq!(ui.close(), vec![("go".into(), Edit::Begin), ("go".into(), Edit::End)]);
}
...
fn model_identity_survives_reorder_and_display_rename() {
    ...
    assert_eq!(ui.scene().unwrap().surface(a.as_str()).unwrap().frame.x, 60.0);
}
