...
fn panel(progress: f64) -> El {
    let a = col![text("Oscillator")].pad(L).size(130., 80.)
        .fill(Gradient::vertical(Primary, Raised)).border(Ink, 2.).radius(18.).id("a");
    let b = col![text("Filter")].pad(L).size(110., 80.)
        .fill(Secondary).border(Warning, 7.).radius(26.).id("b");
    row![a, b].gap(10.).weld_with(Weld::all().reach(20.).blend(70.).morph(progress)).id("rack")
}
