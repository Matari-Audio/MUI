...
/// Explicit opaque conversion at the runtime boundary; alpha needs compositing first.
impl TryFrom<crate::color::Color> for Rgb {
    type Error = ColorError;
    fn try_from(color: crate::color::Color) -> Result<Self, Self::Error> {
        if !color.valid() || color.alpha() != 1.0 {
            return Err(ColorError::InvalidOklch);
        }
        let [r, g, b, _] = color.to_srgb().components;
        Ok(Self(
            (r * 255.).round() as u8,
            (g * 255.).round() as u8,
            (b * 255.).round() as u8,
...
impl From<Rgb> for crate::color::Color {
    fn from(Rgb(r, g, b): Rgb) -> Self {
        Self::srgb(r as f32 / 255., g as f32 / 255., b as f32 / 255.)
    }
}
impl crate::color::Palette {
    /// Resolve source colors, then enforce the runtime's encoded-sRGB contrast contract.
    pub fn resolve_rgb(self) -> Result<Colors, ColorError> {
        if !self.valid() {
            return Err(ColorError::InvalidOklch);
        }
        let canvas = Rgb::try_from(self.background())?;
        let panel = Rgb::try_from(self.surface())?;
        let raised = Rgb::try_from(self.raised())?;
        let backgrounds = [canvas, panel, raised];
        let text = Rgb::try_from(self.ink())?.contrast_on(&backgrounds, 4.5)?;
        let muted = Rgb::try_from(self.dim(self.background()))?.contrast_on(&backgrounds, 4.5)?;
        let outline = Rgb::try_from(self.dim(self.background()))?.contrast_on(&backgrounds, 3.)?;
        let accent = |color: crate::color::Color| -> Result<Accent, ColorError> {
            let fill = Rgb::try_from(color)?;
            let soft = Rgb::try_from(color.mix(self.background(), 0.8))?;
            Ok(Accent {
                fill,
                soft,
                on_fill: Rgb::try_from(self.on(color))?.contrast_on(&[fill], 4.5)?,
                on_soft: text.contrast_on(&[soft], 4.5)?,
            })
        };
        Ok(Colors {
