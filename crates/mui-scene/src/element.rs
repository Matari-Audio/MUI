...
pub trait Paints: Sized {
    ...
    fn border(mut self, paint: impl Into<Fill>, width: f64) -> Self {
        self.style_mut().stroke = Some(Stroke { fill: paint.into(), width: Some(width) });
        self
    }
...
pub trait Styled: Paints {
    ...
    fn weld_shape(self) -> Self { self.weld_with(crate::Weld::shape()) }
    /// Merge borders while preserving the original body coverage and paint.
    fn weld_borders(self) -> Self { self.weld_with(crate::Weld::borders()) }
