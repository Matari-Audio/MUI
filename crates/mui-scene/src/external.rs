...
use crate::{SceneError, ResolvedScene};
...
impl ExternalWeld {
    pub fn bounds(&self)->Bounds {
        ...
        Bounds::new(b.x0+self.origin.x,b.y0+self.origin.y,b.x1+self.origin.x,b.y1+self.origin.y)
    }
    pub fn rect(&self)->Result<RoundedRect,SceneError> {
        Ok(RoundedRect::new(self.bounds(),0.)?)
    }
    pub fn contains(&self,p:Point)->bool {
        self.material.contains(WeldPoint::new(p.x-self.origin.x,p.y-self.origin.y))
    }
}
impl ResolvedScene {
    pub fn external_weld(&self,key:&str)->Option<&ExternalWeld> {self.external_welds.get(key)}
    pub fn external_welds(&self)->impl Iterator<Item=(&str,&ExternalWeld)> + Clone {
        self.external_welds.iter().map(|(k,v)|(k.as_ref(),v))
    }
    pub fn set_weld_solid_material(&mut self,key:&str,index:usize,fill:Option<crate::Color>,border:Option<crate::Color>,width:f64)->Result<bool,SceneError> {
        self.external_welds.get_mut(key)
            .ok_or(SceneError::UnsupportedWeld("key is not a GPU material weld"))?
            .material.set_solid_material(index,fill.map(crate::material_weld::color),border.map(crate::material_weld::color),width)
            .map_err(SceneError::MaterialWeld)
    }
    /// Update only one plate's material, preserving its geometry and domain.
    pub fn set_weld_source_material(&mut self,key:&str,index:usize,source:mui_weld::analytic::AnalyticSource)->Result<bool,SceneError> {
        self.external_welds.get_mut(key)
            .ok_or(SceneError::UnsupportedWeld("key is not a GPU material weld"))?
            .material.set_source_material(index,source).map_err(SceneError::MaterialWeld)
    }
    ...
    pub fn set_weld_morph(&mut self,key:&str,progress:f64)->Result<bool,SceneError> {
        self.external_welds.get_mut(key)
            .ok_or(SceneError::UnsupportedWeld("key is not a GPU material weld"))?
            .material.set_morph(progress).map_err(SceneError::MaterialWeld)
    }
    pub fn set_weld_material_blend(&mut self,key:&str,blend:f64)->Result<bool,SceneError> {
        self.external_welds.get_mut(key)
            .ok_or(SceneError::UnsupportedWeld("key is not a GPU material weld"))?
            .material.set_material_blend(blend).map_err(SceneError::MaterialWeld)
    }
}
...
pub(crate) fn finish_gpu(
    sources:&[mui_weld::Source],weld:mui_weld::Weld,quality:mui_weld::Quality,origin:Point,
    ...
    let work = pixels.checked_mul(material.boundary_segments().max(sources.len()) as u64)
    ...
    if pixels > quality.max_pixels as u64 || work > quality.max_work as u64 {
        return Err(SceneError::UnsupportedWeld("GPU weld exceeds explicit node pixel/work budget"));
    }
    let external=ExternalWeld {origin,material};
    let image_rect=external.rect()?;
    Ok(crate::material_weld::MaterialWeld {
        outline:image_rect.path(),image_fill:crate::Fill::None,image_rect,members,
