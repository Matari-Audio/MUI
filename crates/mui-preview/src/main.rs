...
impl App {
    fn new() -> Self {
        ...
        Self {
            ui: {
                let ui = Ui::new(skin::SKIN).font(font.clone());
                #[cfg(feature = "gpu-effects")]
    ...
    fn draw(&mut self) {
        ...
        let draw_extra = |canvas: &mut mui::vello::Gpu<'_>| {
            if let Some((key, path)) = extra {
                if let (Some(s), Ok(bez)) = (
                    scene.surface(key), mui::vello::bez_path(&path, mui::vello::ARC_TOLERANCE),
                ) {
                    canvas.set_transform(xf * Affine::translate((s.frame.x, s.frame.y)));
                    canvas.set_paint(self.ui.theme.palette.on(self.ui.theme.palette.raised()).to_srgb().into());
                    canvas.fill_path(&bez);
                }
            }
            if self.frames {
                let pointer = self.pointer.pos.map(|p| Point::new(p.x / scale, p.y / scale));
                inspect(canvas, scene, xf, &self.ui.theme.palette, &self.font, pointer, height);
            }
        };
        ...
        if let Err(e) = result { eprintln!("GPU paint: {e}"); }
    }
...
impl ApplicationHandler<AccessEvent> for App {
                ...
                if let Some(key) = self.key_of(r.target_node) {
                    match r.action {
                        ...
                        AccessAction::SetValue => {
                            if let Some(mui_access::accesskit::ActionData::NumericValue(value)) = r.data {
                                self.ui.request_action(SemanticAction::set_value(key, value));
                            }
                        }
                        _ => {}
                    }
    ...
    fn about_to_wait(&mut self, event_loop: &ActiveEventLoop) {
        if !self.visible { event_loop.set_control_flow(ControlFlow::Wait); return; }
        let now = Instant::now();
        ...
        let theme = self.theme_path.as_ref().and_then(|_| self.theme_at.checked_add(POLL));
    ...
    fn window_event(&mut self, event_loop: &ActiveEventLoop, _: WindowId, event: WindowEvent) {
        ...
        match event {
            ...
            WindowEvent::Occluded(hidden) => {
                self.visible = !hidden;
                if hidden { self.repaint_at = None; self.cancel(); }
            }
