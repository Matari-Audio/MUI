...
impl Weld {
    ...
    pub fn crisp() -> Self { Self::all().reach(0.0) }
    /// Opt an existing channel policy into crisp contact welding.
    pub fn sharp(self) -> Self { self.reach(0.0) }

    pub const fn all() -> Self {
        Self { fill: Channel::Blend, border: Channel::Blend, reach: 16.0, blend: 32.0, progress: 1.0 }
    }
    pub const fn shape() -> Self { Self { border: Channel::Keep, ..Self::all() } }
    pub const fn borders() -> Self { Self { fill: Channel::Keep, ..Self::all() } }
    pub const fn fill(mut self, policy: Channel) -> Self { self.fill = policy; self }
    pub const fn border(mut self, policy: Channel) -> Self { self.border = policy; self }
    pub const fn reach(mut self, units: f64) -> Self { self.reach = units; self }
    pub const fn blend(mut self, units: f64) -> Self { self.blend = units; self }
    pub const fn morph(mut self, progress: f64) -> Self { self.progress = progress; self }
    pub(crate) fn validate(self) -> Result<(), Error> {
        if !self.reach.is_finite() || !(0.0..=1e4).contains(&self.reach)
            || !self.blend.is_finite() || !(0.0..=1e4).contains(&self.blend)
            || !self.progress.is_finite() || !(0.0..=1.0).contains(&self.progress) {
...
impl Default for Quality {
    fn default() -> Self { Self { scale: 1.0, max_pixels: 1_048_576, max_work: 100_000_000 } }
}
impl Quality {
    pub fn at_scale(scale: f64) -> Self { Self { scale, ..Self::default() } }
    pub(crate) fn validate(self) -> Result<(), Error> {
        if !self.scale.is_finite() || !(0.125..=8.0).contains(&self.scale)
            || self.max_pixels == 0 || self.max_work == 0 {
...
impl Request {
    pub fn validate(&self) -> Result<(), Error> {
        ...
        for source in &self.sources {
            source.validate()?;
            segments = segments.checked_add(source.shape.cost()).ok_or(Error::Budget)?;
        }
        if segments > 250_000 { return Err(Error::Budget); }
        Ok(())
    }
}
