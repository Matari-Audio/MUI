...
impl Color {
    ...
    pub fn srgb(r: f64, g: f64, b: f64, alpha: f64) -> Self {
        let linear = |x: f64| if x <= 0.04045 { x / 12.92 } else { ((x + 0.055) / 1.055).powf(2.4) };
    ...
    fn from_lab([l,a,b]: [f64;3], alpha: f64) -> Self {
        ...
        Self([4.0767416621*ll - 3.3077115913*mm + 0.2309699292*ss,
             -1.2684380046*ll + 2.6097574011*mm - 0.3413193965*ss,
             -0.0041960863*ll - 0.7034186147*mm + 1.7076147010*ss, alpha])
    }
    ...
    pub(crate) fn weighted(values: impl IntoIterator<Item=(Self,f64)>) -> Self {
        ...
        for (c,w) in values {
            let wa = w*c.0[3];
            let v = c.lab();
            for j in 0..3 { lab[j] += v[j]*wa; }
            alpha += wa;
        }
        if alpha <= 1e-15 { return Self::TRANSPARENT; }
...
impl Premul {
    pub(crate) fn scale(self, x:f64) -> Self { Self(self.0.map(|v| v*x)) }
    pub(crate) fn over(self, bottom:Self) -> Self {
        Self(std::array::from_fn(|j| self.0[j] + bottom.0[j]*(1.0-self.0[3])))
    }
    pub(crate) fn mix(self, other:Self, t:f64) -> Self {
        Self(std::array::from_fn(|j| self.0[j]*(1.0-t) + other.0[j]*t))
    }
    pub(crate) fn straight(self) -> Color {
        let a=self.0[3];
        if a<=1e-15 { Color::TRANSPARENT } else { Color([self.0[0]/a,self.0[1]/a,self.0[2]/a,a]) }
    }
}

#[derive(Clone, Debug, PartialEq)]
pub struct Stop { pub at:f64, pub color:Color }
#[derive(Clone, Copy, Debug, PartialEq, Eq)]
pub enum ImageFit { Fill, Cover, Contain }
#[derive(Clone, Debug)]
pub struct Image { pub width:u32, pub height:u32, pub rgba:Arc<[u8]> }
...
pub enum Brush {
    Solid(Color),
    Linear { from:Point, to:Point, stops:Vec<Stop> },
    Radial { center:Point, radius:f64, stops:Vec<Stop> },
    Conic { center:Point, angle:f64, stops:Vec<Stop> },
    Image { image:Image, bounds:Rect, fit:ImageFit },
}
impl Brush {
    pub(crate) fn validate(&self) -> Result<(),Error> {
        let stops = match self {
            ...
            Self::Linear {from,to,stops} => {
                ...
                if from==to { return Err(Error::Invalid("zero-length gradient")); }
                stops
            }
            Self::Radial {center,radius,stops} => {
                center.validate()?;
                if !radius.is_finite() || *radius<=0.0 || *radius>1e7 { return Err(Error::Invalid("gradient radius")); }
                stops
            }
            Self::Conic {center,angle,stops} => {
                center.validate()?;
                if !angle.is_finite() { return Err(Error::Invalid("gradient angle")); }
                stops
            }
            Self::Image {image,bounds,..} => {
                bounds.validate()?;
                let bytes=(image.width as usize).checked_mul(image.height as usize).and_then(|n|n.checked_mul(4));
                return if image.width==0 || image.height==0 || bytes!=Some(image.rgba.len()) {
                    Err(Error::Invalid("image dimensions"))
                } else { Ok(()) };
            }
        };
        if stops.is_empty() || stops.len()>256 { return Err(Error::Invalid("gradient stop count")); }
        ...
        for s in stops {
            ...
            s.color.validate()?; previous=s.at;
        }
        Ok(())
    }
    ...
    pub(crate) fn retained_bytes(&self) -> usize {
        match self {
            ...
            Self::Image { image, .. } => image.rgba.len().saturating_add(2 * std::mem::size_of::<usize>()),
        }
    }
    pub fn sample(&self,p:Point) -> Color {
        match self {
            ...
            Self::Radial {center,radius,stops} => ramp(stops,(p-*center).length()/radius),
            Self::Conic {center,angle,stops} => {
                let d=p-*center;
                // CSS angles: zero points up, positive turns clockwise.
                let a=d.y.atan2(d.x)+std::f64::consts::FRAC_PI_2-angle.to_radians();
                ramp(stops,a.rem_euclid(std::f64::consts::TAU)/std::f64::consts::TAU)
            }
...
fn ramp(stops:&[Stop], t:f64) -> Color {
    ...
    if right>=stops.len() { return stops[stops.len()-1].color; }
    if right==0 { return first.color; }
...
fn image_sample(image:&Image,b:Rect,fit:ImageFit,p:Point) -> Color {
    ...
    if image.width==0 || image.height==0 || b.validate().is_err() || p.validate().is_err() { return Color::TRANSPARENT; }
    let (iw,ih)=(f64::from(image.width),f64::from(image.height));
    let (mut w,mut h)=(b.width(),b.height());
    if fit!=ImageFit::Fill {
        let k=if fit==ImageFit::Cover {(w/iw).max(h/ih)} else {(w/iw).min(h/ih)};
        w=iw*k; h=ih*k;
    }
    let (u,v)=((p.x-(b.x0+(b.width()-w)/2.0))/w,(p.y-(b.y0+(b.height()-h)/2.0))/h);
    if fit==ImageFit::Contain && (!(0.0..=1.0).contains(&u)||!(0.0..=1.0).contains(&v)) { return Color::TRANSPARENT; }
    let (x,y)=((u*iw-0.5).clamp(0.0,iw-1.0),(v*ih-0.5).clamp(0.0,ih-1.0));
    let (ix,iy)=(x.floor() as u32,y.floor() as u32);
    let read=|xx:u32,yy:u32| {
        let offset=(yy as usize).checked_mul(image.width as usize).and_then(|n|n.checked_add(xx as usize)).and_then(|n|n.checked_mul(4));
        let Some(bytes)=offset.and_then(|i| i.checked_add(4).and_then(|end| image.rgba.get(i..end))) else { return Premul::default() };
        Color::srgb(bytes[0] as f64/255.0,bytes[1] as f64/255.0,bytes[2] as f64/255.0,bytes[3] as f64/255.0).premul()
    };
    let nx=ix.saturating_add(1).min(image.width-1); let ny=iy.saturating_add(1).min(image.height-1);
    read(ix,iy).mix(read(nx,iy),x-f64::from(ix)).mix(read(ix,ny).mix(read(nx,ny),x-f64::from(ix)),y-f64::from(iy)).straight()
}
