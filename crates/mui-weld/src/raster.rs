...
impl Baked {
    pub fn bytes(&self)->usize {
        self.contours.iter().fold(
            self.rgba.len().saturating_add(std::mem::size_of::<Self>())
                .saturating_add(self.contours.capacity().saturating_mul(std::mem::size_of::<Vec<Point>>())),
            |bytes, c| bytes.saturating_add(c.capacity().saturating_mul(std::mem::size_of::<Point>())),
        )
    }
}
...
pub fn bake(request:&Request)->Result<Baked,Error> {
    ...
    let boundary=if request.weld.reach==0.0 && request.sources.len()<=3 {
        let plates=request.sources.iter().map(|s|match &s.shape {
            crate::Geometry::RoundedRect{bounds,radius}=>Some(crate::boundary::Plate {
                center:Point::new((bounds.x0+bounds.x1)*0.5,(bounds.y0+bounds.y1)*0.5),
                half:Point::new(bounds.width()*0.5,bounds.height()*0.5),radius:*radius,angle:0.0,
            }),_=>None,
        }).collect::<Option<Vec<_>>>();
        plates.map(crate::boundary::Boundary::new).transpose()?
    }else{None};
    let mut ext=request.sources[0].shape.bounds().ok_or(Error::Invalid("source bounds"))?;
    for s in request.sources.iter().skip(1) {ext=ext.union(s.shape.bounds().ok_or(Error::Invalid("source bounds"))?);}
    let px=1.0/q.scale;
    ...
    if !(1.0..=4096.0).contains(&wf)||!(1.0..=4096.0).contains(&hf) {return Err(Error::Budget);}
    ...
    let cost=request.sources.iter().map(|s|s.shape.cost()+32).sum::<usize>()+boundary.as_ref().map_or(0,|b|b.edges().len()*4);
    let work=pixels.checked_add(vertices).and_then(|n|n.checked_mul(cost)).ok_or(Error::Budget)?;
    if pixels>q.max_pixels||work>q.max_work {return Err(Error::Budget);}
    ...
    for y in 0..=h {for x in 0..=w {
        ...
        let d=boundary.as_ref().map_or_else(||field(&request.sources,p,request.weld).distance,|b|b.sample(p).distance);
        if !d.is_finite() {return Err(Error::Invalid("non-finite field"));}
    ...
    for y in 0..h {for x in 0..w {
        ...
        let c=pixel_prepared(&request.sources,p,request.weld,px,boundary.as_ref()).rgba8();
        rgba[4*(y*w+x)..4*(y*w+x)+4].copy_from_slice(&c);
    }}
    Ok(Baked{bounds,width:w as u32,height:h as u32,rgba:Arc::from(rgba),contours})
}

type Edge=(usize,usize);
fn edge(a:usize,b:usize)->Edge {if a<b {(a,b)} else {(b,a)}}
...
fn contours(values:&[f64],w:usize,h:usize,b:Rect)->Result<Vec<Vec<Point>>,Error> {
    let sx=b.width()/w as f64; let sy=b.height()/h as f64;
    let position=|i:usize|Point::new(b.x0+(i%(w+1)) as f64*sx,b.y0+(i/(w+1)) as f64*sy);
    let mut next:BTreeMap<Edge,(Edge,Point)>=BTreeMap::new();
    let mut incoming=BTreeSet::new();
    for y in 0..h {for x in 0..w {
        let a=y*(w+1)+x;let d=a+w+1;
        for ids in [[a,a+1,d+1],[a,d+1,d]] {
            let mut cuts = [((0usize, 0usize), Point::default()); 2];
            let mut cut_count = 0;
            for j in 0..3 {
                let (u,v)=(ids[j],ids[(j+1)%3]);
                if (values[u]<0.0)==(values[v]<0.0) {continue;}
                let t=values[u]/(values[u]-values[v]);
                if cut_count == cuts.len() { return Err(Error::OpenContour); }
                cuts[cut_count] = (edge(u,v),position(u)+(position(v)-position(u))*t);
                cut_count += 1;
            }
            if cut_count == 0 {continue;}
            if cut_count != 2 {return Err(Error::OpenContour);}
            let inside=*ids.iter().find(|&&i|values[i]<0.0).ok_or(Error::OpenContour)?;
            ...
            if (z.1-a.1).cross(position(inside)-a.1)<0.0 {std::mem::swap(&mut a,&mut z);}
            if next.insert(a.0,(z.0,a.1)).is_some()||!incoming.insert(z.0) {return Err(Error::OpenContour);}
        }
    }}
    let mut out=Vec::new();
