...
pub(crate) fn floor(s: &str, at: usize) -> usize {
    let b = byte(s, at);
    let boundary = s.grapheme_indices(true).map(|(i, _)| i)
        .chain(std::iter::once(s.len())).take_while(|&i| i <= b).last().unwrap_or(0);
    scalar(s, boundary)
}
pub(crate) fn previous(s: &str, at: usize) -> usize {
    let b = byte(s, at);
    let boundary = s.grapheme_indices(true).map(|(i, _)| i)
        .take_while(|&i| i < b).last().unwrap_or(0);
    scalar(s, boundary)
}
pub(crate) fn next(s: &str, at: usize) -> usize {
    let b = byte(s, at);
    let boundary = s.grapheme_indices(true).map(|(i, _)| i)
        .find(|&i| i > b).unwrap_or(s.len());
    scalar(s, boundary)
}
