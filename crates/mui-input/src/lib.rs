...
struct Target {
    ...
    clip: Option<Bounds>,
    /// All painted ancestor clips, including holes and rounded corners.
...
impl Hit {
    ...
    pub fn push_clipped(
        ...
        self.push_in_clips(id, None, path, clip, &[])
    }
    ...
    pub fn push_tagged(
        ...
        self.push_in_clips(id, Some(tag.into()), path, clip, &[])
    }

    /// Register a target with the same ancestor clip paths used for painting.
    /// `clip` is only a cheap rectangular rejection; every path must also contain
    /// the pointer. Floated content passes an empty path list when it escapes clips.
    pub fn push_in_clips(
        &mut self,
        id: impl Into<String>,
        tag: Option<String>,
        path: &Path,
        ...
        let clip_paths = clip_paths
            .iter()
            .map(|p| mui_geometry::bez_path(p, mui_geometry::ARC_TOLERANCE))
    ...
    pub fn at_tagged_with(
        ...
        self.targets
            ...
            .find(|t| {
                inside(&t.clip)
                    && t.bounds.contains(q)
                    && contains(&t.id, t.tag.as_deref(), p).unwrap_or_else(|| t.path.winding(q) != 0)
                    && t.clip_paths.iter().all(|c| c.winding(q) != 0)
            })
...
impl Interaction {
    ...
    pub fn update_with(
        &mut self, hit: &Hit, input: PointerInput,
        contains: impl Fn(&str, Option<&str>, Point) -> Option<bool>,
    ) {
        ...
        let over = input.pos.and_then(|p| hit.at_tagged_with(p, &contains))
