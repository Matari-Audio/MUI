...
fn all_clips_must_accept_the_point() {
    ...
    hit.push_tagged_paths("control", "body", &square, None, Some(&[circle, inner]))
        .unwrap();
...
fn a_clip_hole_does_not_respond() {
    ...
    let mut hit = Hit::default();
    hit.push_clipped_paths("control", &square, None, Some(&[ring]))
        .unwrap();
...
fn malformed_clip_cannot_publish_a_partial_target() {
    ...
    let mut hit = Hit::default();
    assert!(hit
        .push_clipped_paths("bad", &square, None, Some(&[invalid]))
        .is_err());
    assert!(hit.is_empty());
}
