...
def svg():
 ...
 for i,m in enumerate(marks):
  ...
  if m['gradient']:
   b=m['gradient']; parts.append(f'<defs><linearGradient id="g{i}" x1="0" y1="0" x2="1" y2="0"><stop stop-color="{fill}" stop-opacity="{opacity}"/><stop offset="1" stop-color="rgb({b[0]},{b[1]},{b[2]})" stop-opacity="{b[3]/255}"/></linearGradient></defs>'); fill=f'url(#g{i})';opacity=1
  if m['clip']:
...
for image_path in sorted(out.glob('*.png')):
 ...
 data={'scale':scale,'reference':'Cairo 4x supersampling + box downsample, shared cubic paths; not ground truth','rgb_mae':float(d.mean()),'rgb_p99':float(np.percentile(d,99)),'pixels_over_16':float((d.max(2)>16).mean()),'regions':{}}
 ...
 Image.fromarray(np.clip(d*6,0,255).astype('uint8')).save(out/f'diff-{image_path.name}')
