...
| 23 | Accessibility for a 200-control editor | `mui-access::tree_update(scene, focus)` (`access/lib.rs:90`), `A11y::{Button,Toggle,Slider,TextInput}` set by every widget (`element.rs`). **Partial**: no `A11y` for a group/list/menu/canvas, and no live region for the readouts | **Partial** |
