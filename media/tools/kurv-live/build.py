...
if not (build/'.mui-motion-export').is_file():p.error('Run tools/kurv-motion/export.py to initialize an isolated build first.')
from runpy import run_path
run_path(str(ROOT/'tools/kurv-motion/export.py'))['install_capture_runtime'](dst)
gallery=dst/'src/editors/mui2/gallery.rs';s=gallery.read_text();s=s[:s.index('// Included only in an isolated Kurv storybook build')]+(ROOT/'tools/kurv-motion/capture.rs').read_text()+'\n'+(ROOT/'tools/kurv-live/bridge.rs').read_text();gallery.write_text(s)
